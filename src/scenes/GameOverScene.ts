...
export class GameOverScene extends Phaser.Scene {
  ...
  create() {
    ...
    this.add.text(WIDTH - 38, 22, `MELHOR  ${String(this.high).padStart(4, "0")}`, TEXT_PRESETS.monoLabel).setOrigin(1, 0);

    this.add.text(22, HEIGHT - 22, "GAMEDEV.05 · GAME OVER", TEXT_PRESETS.hint).setOrigin(0, 1);

    this.add
      .text(WIDTH / 2, 150, this.isNewRecord ? "NOVO RECORDE" : "GAME OVER", TEXT_PRESETS.monoLabel)
      .setOrigin(0.5)
      .setColor(this.isNewRecord ? COLORS.accent : COLORS.muted);

    this.add
      .text(WIDTH / 2, 230, String(this.score).padStart(4, "0"), TEXT_PRESETS.heroOutline)
      .setOrigin(0.5)
      .setFontSize("128px");

    this.add
      .text(WIDTH / 2, 340, `onda ${this.wave}  ·  melhor ${this.high}`, TEXT_PRESETS.body)
      .setOrigin(0.5);
